...
func load_level_from_scene(scene: PackedScene):
	...
	update_hex_display()
	hex_display.update_level_specific_labels(loaded_level.validator.desc, loaded_level.is_solvable_level)
...
func reload_current_level(reset_border_score: bool, same_seed: bool = true):
	...
	# Update hex_display
	update_hex_display()
	#hex_display.update_level_specific_labels(loaded_level.validator.desc, loaded_level.is_solvable_level) # Level desc shouldn't change
...
func transition_from_world(selected_level: PackedScene):
	...
	hex_display.update_level_specific_labels(new_level.validator.desc, new_level.is_solvable_level)
...
func validate_level():
	if loaded_level.has_method("validate"):
		...
		hex_display.update_level_specific_labels(loaded_level.validator.desc, loaded_level.is_solvable_level, false) # Refresh level desc, as validator can sometimes change this.
	else:
